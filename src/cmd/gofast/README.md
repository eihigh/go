...
- 現在の `gofast` は **出力バイナリ全体のキャッシュ再利用** 方式です。
- そのため、`「ユーザーランドのコードだけ再コンパイルし、標準ライブラリ部分のバイナリだけをコピーして再結合する」` 方式には対応していません。
- キャッシュミス時は部分再利用ではなく、通常の `go build` を実行して新しい出力バイナリを生成します。
